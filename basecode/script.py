...
def mlrObjFunction(params, *args):
    ...
    bias_term = np.ones((n_data, 1)) 
    x = np.concatenate((bias_term, train_data),1) 
    w = params.reshape((n_feature+1,n_class))   
    theta = np.zeros((n_feature+1,n_class))

    y = np.exp(np.dot(x,w)) 
    z = np.sum((a),axis=1).reshape(n_data,1)
    theta = np.divide(y,z) 

    error = np.multiply(-1,np.sum(np.multiply(labeli,np.log(theta))))/n_data
    error_grad = ((np.dot(np.transpose(x),np.subtract(theta,labeli)))/n_data).flatten() 
